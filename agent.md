...
- 2026-04-13: Refatoração do endpoint `api/checkout.js`: migração para ESM (`import`), inicialização com `apiVersion`, e tratamento global de erros para garantir respostas JSON consistentes.
- 2026-04-13: Correção da infraestrutura serverless: migração para `export default` no `api/checkout.js`, adição de headers CORS e tratamento de erro robusto no frontend.
